...
namespace gameplay_old {
	...
	bool Game::manageCheckState(std::array<chessgame::Coordinates, 2>& move) {
		...
		bool is_passant = this->isEnPassant(p->getSymbol(), move[1]); // ATTENZIONE: sto resettando l'en-passant
	...
	// o tutte mosse consentite
	// oppure tutte mosse consentite solo per il check

	std::vector<chessgame::Coordinates> Game::checkStateAllowedMoves() {
		std::vector<std::array<chessgame::Coordinates,2>> allowed_moves;
